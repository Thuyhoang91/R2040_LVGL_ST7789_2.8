...
#include "lv_port_disp.h" // For GPIO functions

// SPI configuration for XPT2046 (typically slower than display)
#define XPT_SPI_BAUD_RATE (0.01 * 1000 * 1000) // 2 MHz, XPT2046 max is often around 2.5MHz
...
bool xpt2046_get_touch_point(uint16_t *x, uint16_t *y) {
    ...
    if (LV_PORT_DISP_ROTATION_VALUE == LV_PORT_ROT_NONE) {
    ...
    } else if (LV_PORT_DISP_ROTATION_VALUE == LV_PORT_ROT_90) {
        // 90 degrees rotation, swap axes
        cal_x = LV_VER_RES_MAX - ( (int32_t)raw_y - XPT2046_MIN_RAW_Y ) * LV_VER_RES_MAX / (XPT2046_MAX_RAW_Y - XPT2046_MIN_RAW_Y);
        cal_y = LV_HOR_RES_MAX - ( (int32_t)raw_x - XPT2046_MIN_RAW_X ) * LV_HOR_RES_MAX / (XPT2046_MAX_RAW_X - XPT2046_MIN_RAW_X);
    } else if (LV_PORT_DISP_ROTATION_VALUE == LV_PORT_ROT_180) {
        // 180 degrees rotation, invert both axes
        cal_x = ( (int32_t)raw_x - XPT2046_MIN_RAW_X ) * LV_HOR_RES_MAX / (XPT2046_MAX_RAW_X - XPT2046_MIN_RAW_X);
        cal_y = LV_VER_RES_MAX - ( (int32_t)raw_y - XPT2046_MIN_RAW_Y ) * LV_VER_RES_MAX / (XPT2046_MAX_RAW_Y - XPT2046_MIN_RAW_Y);
    } else if (LV_PORT_DISP_ROTATION_VALUE == LV_PORT_ROT_270) {
        // 270 degrees rotation, swap axes again
        cal_x = ( (int32_t)raw_y - XPT2046_MIN_RAW_Y ) * LV_VER_RES_MAX / (XPT2046_MAX_RAW_Y - XPT2046_MIN_RAW_Y);
        cal_y = ( (int32_t)raw_x - XPT2046_MIN_RAW_X ) * LV_HOR_RES_MAX / (XPT2046_MAX_RAW_X - XPT2046_MIN_RAW_X);
    }
        // Default case, no rotation or unknown rotation
    //cal_x = LV_HOR_RES_MAX - ( (int32_t)raw_x - XPT2046_MIN_RAW_X ) * LV_HOR_RES_MAX / (XPT2046_MAX_RAW_X - XPT2046_MIN_RAW_X);
    ...
    *y = (uint16_t)cal_y;
    ...
    if (LV_PORT_DISP_ROTATION_VALUE == LV_PORT_ROT_NONE) {
        // No rotation, standard clamping
        if (*x >= LV_HOR_RES_MAX) *x = LV_HOR_RES_MAX - 1;
        if (*y >= LV_VER_RES_MAX) *y = LV_VER_RES_MAX - 1;
    } else if (LV_PORT_DISP_ROTATION_VALUE == LV_PORT_ROT_90) {
        // 90 degrees rotation, swap clamping
        if (*x >= LV_VER_RES_MAX) *x = LV_VER_RES_MAX - 1;
        if (*y >= LV_HOR_RES_MAX) *y = LV_HOR_RES_MAX - 1;
    } else if (LV_PORT_DISP_ROTATION_VALUE == LV_PORT_ROT_180) {
        // 180 degrees rotation, standard clamping
        if (*x >= LV_HOR_RES_MAX) *x = LV_HOR_RES_MAX - 1;
        if (*y >= LV_VER_RES_MAX) *y = LV_VER_RES_MAX - 1;
    } else if (LV_PORT_DISP_ROTATION_VALUE == LV_PORT_ROT_270) {
        // 270 degrees rotation, swap clamping
        if (*x >= LV_VER_RES_MAX) *x = LV_VER_RES_MAX - 1;
        if (*y >= LV_HOR_RES_MAX) *y = LV_HOR_RES_MAX - 1;
    }
    // if (*x >= LV_HOR_RES_MAX) *x = LV_HOR_RES_MAX - 1;
